...
mixin WebviewMixin {
  ...
  Future<NavigationDecision> getNavigationDelegate(
    ...
    if (isInitialLoad) {
      // Skip the initial load navigation delegate logic
      printLog('[WebView] Initial load, skipping navigation logic.');
      // return NavigationDecision.navigate;
      printLog('[WebView] Initial load,done going next.');
      printLog('[WebView] done intial');

    }

    if (!isInitialLoad) {
      if (url.contains('contact')) {
       _openInNewTab(context, url);
        return NavigationDecision.prevent;
      } else if (url.contains('a-propos')) {
       _openInNewTab(context, url);
        return NavigationDecision.prevent;
      } else if (url.contains('services')) {
        MainTabControlDelegate.getInstance().tabAnimateTo(1);
        return NavigationDecision.prevent;
      } else if (url.contains('nos-realisations')) {
  ...
  void _openInNewTab(BuildContext context, String url) {
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          onNavigationRequest: (request) async {
              return await getNavigationDelegate(request, context);
            },
